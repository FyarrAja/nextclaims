...
import net.william278.nextclaims.claim.ClaimingMode;
import net.william278.nextclaims.database.Database;
import net.william278.nextclaims.highlighter.Highlightable;
import net.william278.nextclaims.network.Broker;
import net.william278.nextclaims.position.World;
import net.william278.nextclaims.util.BlockProvider;
...
public final class Settings {
    ...
    static final String CONFIG_HEADER = """
            ...
            ┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
            ┣╸ Information: https://william278.net/project/NextClaims/
            ┣╸ Config Help: https://william278.net/docs/NextClaims/config/
            ┗╸ Documentation: https://william278.net/docs/NextClaims/""";

    @Comment("Locale of the default language file to use. Docs: https://william278.net/docs/NextClaims/translations")
    ...
    @Comment("How many seconds players must wait between executing NextClaims commands (floating point number)")
    ...
    public static class CrossServerSettings {
        ...
        @Comment({"The cluster ID, for if you're networking multiple separate groups of nextclaims-enabled servers.",
    ...
    public static class ClaimSettings {
        ...
        @Comment({"The number of claim blocks a user gets hourly.",
                "Override with the \"NextClaims.hourly_blocks.(amount)\" permission"})
        private long hourlyClaimBlocks = 100;

        @Comment({"The maximum number of claim blocks a user can have.",
                "Override with the \"NextClaims.max_claim_blocks.(amount)\" permission"})
        ...
        @Comment("Give claim tools (claim tool and inspection tool) to players on first join")
        private boolean giveToolsOnJoin = true;
    ...
    @Comment("Settings for protecting tamed animals (pets). Docs: https://william278.net/docs/NextClaims/pets")
    ...
    public static class ModerationSettings {
        ...
        private WarnSettings warnings = new WarnSettings();

        @Getter
        @Configuration
        @NoArgsConstructor(access = AccessLevel.PRIVATE)
        public static class WarnSettings {
            @Comment("Whether to enable warn notification when players enter claims they are warned about")
            private boolean enabled = false;

            @Comment("Whether to broadcast warnings to all online staff members")
            private boolean broadcastToStaff = true;

            @Comment("The permission required to receive warn notifications")
            private String notifyPermission = "NextClaims.staff.warn_notify";
        }
        ...
        public static class ToolSettings {
            @Comment("Display name for the claim tool given to players on join (supports color codes with &)")
            private String claimToolDisplayName = "&6&lLand Claim Wand";

            @Comment("Display name for the inspection tool given to players on join (supports color codes with &)")
            private String inspectionToolDisplayName = "&b&lLand Info Wand";

            @Comment("The item name displayed in tool messages (e.g., 'Land Claim Wand', 'Golden Shovel')")
            private String claimToolItemName = "Land Claim Wand";

            @Comment("The item name displayed in tool messages (e.g., 'Land Info Wand', 'Stick')")
            private String inspectionToolItemName = "Land Info Wand";
        }
    ...
    public static class HookSettings {
        ...
        public static class LuckPermsHookSettings {
            ...
            @Comment("Require users to have the \"NextClaims.trust.luckperms\" permission to use LuckPerms trust tags")
        ...
        public static class PlaceholderSettings {
            @Comment("Whether to hook into PlaceholderAPI to provide a NextClaims placeholder expansion")
            private boolean enabled = true;
        }
