...
package net.william278.nextclaims.claim;
...
public class RegionSerializer implements JsonSerializer<Region>, JsonDeserializer<Region> {
    ...
    private final NextClaims plugin;
    ...
    public Region deserialize(JsonElement jsonElement, Type type, JsonDeserializationContext jsonDeserializationContext) throws JsonParseException {
        ...
        // Validate array has exactly 4 elements (x1, z1, x2, z2)
        if (jsonArray.size() < 4) {
            throw new JsonParseException("Region deserialization failed: expected 4 elements [x1, z1, x2, z2], but found " + jsonArray.size());
        }
