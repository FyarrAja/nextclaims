...
package net.william278.nextclaims.moderation;
...
import net.william278.nextclaims.user.User;
...
public interface DropsHandler {
    ...
    default void checkDroppedItem(@NotNull GroundStack item) {
        ...
        removeIfMarkedDropper(item.getStack()).ifPresent(owner -> {
            lockDrop(owner, item);
            // Safely remove the item from marked drops with null check
            final List<DroppedItem> markedDrops = getMarkedDrops().get(owner);
            if (markedDrops != null) {
                markedDrops.remove(item.getStack());
            }
        });
    }
